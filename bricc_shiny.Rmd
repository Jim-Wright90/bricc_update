...
bricc$cohort <- factor(bricc$cohort, levels = c("2017-2019",
                                                "2018-2020",
                                                "2019-2021",
                                                "2020-2022",
                                                "2021-2023"))
...
bricc_demo$cohort <- factor(bricc_demo$cohort, levels = c("2017-2019",
                                                "2018-2020",
                                                "2019-2021",
                                                "2020-2022",
                                                "2021-2023"))
...
bricc_1$cohort <- factor(bricc_1$cohort, levels = c("2017-2019",
                                                "2018-2020",
                                                "2019-2021"))
...
bricc_2$cohort <- factor(bricc_2$cohort, levels = c("2019-2021",
                                                "2020-2022",
                                                "2021-2023"))
...
bricc_1_a$cohort <- factor(bricc_1_a$cohort, levels = c("2017-2019",
                                                "2018-2020",
                                                "2019-2021"))
...
bricc_2_a$cohort <- factor(bricc_2_a$cohort, levels = c("2019-2021",
                                                "2020-2022",
                                                "2021-2023"))
...
cohort_total <- bricc_demo %>% 
  count(cohort)
  
group_order <- c("Winter 2022",
                 "Fall 2021 2nd Years",
                 "Fall 2021 1st Years",
                 "Summer 2021",
                 "Spring 2021",
...
student_list <- bricc_demo %>% 
  select(student_name, cohort, group, term) 
...
# Assessment Results 2018-2019 {data-icon="fa-question"}
...
cohort_means_1 <- bricc_1_a %>% 
  select(student_id, student_name, cohort, group, term, assessment, score) %>% 
...
```{r, include=TRUE}
selectInput("cohorts1",
            "Cohort:",
            choices = levels(cohort_means_1$cohort))
...
renderPlotly({
  cohort_means_1 %>% 
    filter(cohort %in% input$cohorts1) %>% 
    ggplot(aes(assessment, average)) +
      geom_col(fill = "#56B4E9", 
           alpha = 0.7) +
  scale_x_discrete(limits = test_order_1) +
  ...
  labs(x = "",
       y = "Score",
       title = paste("Average Scores:", input$cohorts1))
})
...
# Assessment Results 2020-Present {data-icon="fa-question"}
...
```{r, include=TRUE}
selectInput("cohorts2",
            "Cohort:",
            choices = levels(cohort_means_2$cohort))
...
renderPlotly({
  cohort_means_2 %>% 
    filter(cohort %in% input$cohorts2) %>% 
    ggplot(aes(assessment, average)) +
      geom_col(fill = "#56B4E9", 
           alpha = 0.7) +
  scale_x_discrete(limits = test_order_2) +
  ...
  labs(x = "",
       y = "Score",
       title = paste("Average Scores:", input$cohorts2))
})

```


# Survey Results {data-icon="fa-chart-bar"}
...
Pre-survey results not obtained from:

  * Isabel Butler (Fall 2018 2nd Year)



Post-survey results not obtained from:

  * Rhianna Stuck (Summer 2018)
  * Isabel Butler (Fall 2018 2nd Year)
  * Katrina Viers (Fall 2018 1st Year)
...
```{r, include=FALSE}
sq_df <- function(df, a, b, c, d, e, f, g) {
  df %>% 
...
cohort_plot <- function(df) {
  ...
  df %>% 
  ggplot(aes(Response)) +
  geom_bar(aes(fill = cohort), alpha = 0.9) +
  scale_x_discrete(limits = sv_rep) +
